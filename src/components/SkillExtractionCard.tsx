...
const DEMAND_CONFIG = {
  high:   { emoji: '🔥', label: 'High demand', className: 'demandHigh' },
  medium: { emoji: '📈', label: 'Growing',     className: 'demandMedium' },
  low:    { emoji: '⚠️', label: 'Niche',        className: 'demandLow' },
} as const;
...
export function SkillExtractionCard({ evidence, extractedSkills }: Props) {
  ...
  return (
    <article className={styles.card} aria-label={`${typeLabels[evidence.type]}: ${evidence.title} — ${evidenceSkills.length} skill${evidenceSkills.length !== 1 ? 's' : ''} extracted`}>
            ...
            return (
              <div key={idx} className={styles.skill} role="listitem">
                <div className={styles.skillTop}>
                  ...
                  >
                    <span aria-hidden="true">{demandCfg.emoji}</span> {demandCfg.label}
                  </span>
                </div>
