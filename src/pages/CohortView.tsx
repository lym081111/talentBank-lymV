...
export function CohortView({ cohort, readinessProfile, studentProfile, onBack, backLabel }: Props) {
  ...
  const [localSubmissionCount, setLocalSubmissionCount] = useState(() => getCohortSubmissionCount());
  ...
  async function handleSubmitToCohort() {
    ...
    setSubmitted(true);
    setSubmitting(false);
    setLocalSubmissionCount((prev) => prev + 1);
  }
        ...
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="text-xs font-black uppercase tracking-[0.22em] text-cyan-200">
              University View · University Cohort Insight (Stage 1)
            </div>
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            ...
            <div className="mt-3 text-2xl font-black">
              {cohort.totalStudents + (submitted ? 1 : 0)} students
              {submitted && (
                <span className="ml-2 text-sm font-bold text-cyan-300">+1 new</span>
              )}
            </div>
            <p className="mt-2 text-sm text-white/55">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-white/50 max-w-xl">
                ...
                {localSubmissionCount > 0 && (
                  <span className="text-cyan-200 font-bold">{localSubmissionCount} submission{localSubmissionCount !== 1 ? 's' : ''} stored locally.</span>
                )}
              </p>
            </div>
            <button
