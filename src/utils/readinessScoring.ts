...
function scoreProductionPractices(evidence: Evidence[]): DimensionScore {
  ...
  // Cap at 50 to ensure this stays as a gap
  score = Math.min(score, 50);

  return {
    dimension: 'Production Practices',
    score: Math.max(25, Math.min(score, 50)),
    status: score >= 50 ? 'Internship-Ready' : 'Developing',
    explanation:
      score >= 50
        ? 'Some evidence of production practices. Add more testing, CI/CD, and deployment.'
        : 'Limited production practice evidence. Add testing, deployment, or CI/CD to projects.',
    topSkills: [],
  };
}
